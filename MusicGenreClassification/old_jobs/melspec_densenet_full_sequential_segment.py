...
for kfold, (train_index, test_index) in enumerate(skf.split(dataset.X, dataset.Y)):
    ...
    info = {}
    tb_writer = SummaryWriter('runs/densenet_full_sequential_segment_fold_%d' % kfold)
    ...
    with open('checkpoints/densenet_full_sequential_segment_fold_%d.pth' % kfold, 'wb') as f:
