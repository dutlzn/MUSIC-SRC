...
def get_dataloaders(args):
    train_set = GTZAN(phase='train', min_segments=args['train_segments'],
                      randomized=True, overlap=args['overlap'])
    val_set = GTZAN(phase='val', min_segments=args['inference_segments'],
                    overlap=args['overlap'])
    test_set = GTZAN(phase='test', min_segments=args['inference_segments'],
                    overlap=args['overlap'])
...
def get_model(args):
    net = DenseInception(1, 32, 3, num_classes=GTZAN.NUM_CLASSES).to(args['device'])
